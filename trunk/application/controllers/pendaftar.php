...
class Pendaftar extends CI_Controller
	...
	//		c. click kat "pendaftar" -> sbb function ni dlm controller pendaftar
	...
	//			cuba isi dalam format ni "pendaftar : senarai_pemohon : senaraikan semua pemohon dari tarikh a smpi tarikh b"
	//3. dah siap bhg userr access level sbb nak bg admin mencapai controller pendaftar dan function senarai_pemohon
					...
					if(user_role($this->session->userdata('id_user'), $this->uri->segment(1, 0), $this->uri->segment(2, 0)) === TRUE)		//hanya user yang logged in DAN yang berkenaan shj boleh access function senarai_pemohon
						{
							...
							//9. create ./application/views/pendaftar/senarai_pemohon.php, sila lhat fal tu..aku dah komen sedetail mungkin utk hang senang paham...
							...
							if ($this->form_validation->run() == FALSE)																		//bhg ni kalau user belum lg click submit button i.e bukak page ada form
								{
									...
									$this->load->view('pendaftar/senarai_pemohon', $data);													//buat paparan file ./application/views/pendaftar/senarai_pemohon.php yang mana variable "$data" (variable array) dari controller akan di"pas"kan ke views/pendaftar/senarai_pemohon.php
								...
								else
								{
									...
									$this->load->view('pendaftar/senarai_pemohon', $data);													//buat paparan balik jerrrr.....
								}
						}
						else
						{
